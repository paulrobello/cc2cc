...
function SectionHeader({ label, onRemoveAll }: { label: string; onRemoveAll?: () => void }) {
  return (
    <li
      className="flex items-center justify-between px-2.5 pb-0.5 pt-3"
      aria-hidden="true"
    >
      <span
      ...
      {onRemoveAll && (
        <button
          type="button"
          onClick={onRemoveAll}
          className="font-mono text-[9px] uppercase tracking-wider transition-colors duration-100"
          style={{ color: "#2a5480" }}
          onMouseEnter={(e) => { (e.currentTarget as HTMLButtonElement).style.color = "#ef4444"; }}
          onMouseLeave={(e) => { (e.currentTarget as HTMLButtonElement).style.color = "#2a5480"; }}
          aria-label="Remove all offline instances"
        >
          remove all
        </button>
      )}
    ...
    >
      ...
      <ScrollArea className="flex-1 overflow-hidden">
        <ul className="space-y-px p-2" role="list">
          ...
          {offline.length > 0 && (
            <>
              <SectionHeader label="Offline" onRemoveAll={onRemoveAll} />
              {offline.map((inst) => {
                const isSelected = selectedId === inst.instanceId;
                return (
                  <li key={inst.instanceId} className="group flex items-stretch">
